    ...
    {//Line to construct rPr changes 2666
      if (node is XElement element)
      {
        ...
        /*if (element.Name == W.rPr)
        {
          ...
          if (status == "Inserted")
          {
            XElement rPr = new XElement(W.rPr, element.Name,
             element.Attributes());
            ...
            XElement rPrParent = new XElement(element.Name,
             element.Attributes(), rPrChange);
            rPr.Add(element.Nodes().Select(n => MarkContentAsDeletedOrInsertedTransform(n, settings)));
            rPrChange.Add(rPr);
            return rPrParent;
          }
