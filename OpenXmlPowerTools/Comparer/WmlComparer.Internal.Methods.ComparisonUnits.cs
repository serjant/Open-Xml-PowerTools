...
namespace OpenXmlPowerTools
{
  public static partial class WmlComparer
  {
        ...
        WmlComparerSettings settings)
    {
      ...
      {
        IEnumerable<XElement> runPropsChildrenToProcess = element
            .Elements()
            .Where(e => e.Name != W.rPrChange);
        foreach (XElement item in runPropsChildrenToProcess)
        {
          var rPrPropComparisonUnitAtom = new ComparisonUnitAtom(
              item,
              element.AncestorsAndSelf()
                  .TakeWhile(a => a.Name != W.body && a.Name != W.footnotes && a.Name != W.endnotes).Reverse()
                  .ToArray(),
              part,
              settings);
          comparisonUnitAtomList.Add(rPrPropComparisonUnitAtom);
        }
        return;
      }
