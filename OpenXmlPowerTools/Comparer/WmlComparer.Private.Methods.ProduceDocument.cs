...
namespace OpenXmlPowerTools
{
    public static partial class WmlComparer
    {
        ...
        {//Line to construct rPr changes 2666
            if (node is XElement element)
            {
                ...
                {
                    var rPrElement = new XElement(element.Name, element.Attributes());
                    XElement rPrChangeRunPropertyElement = null;
                    foreach (var childRunProperty in element.Nodes().Select(n => n))
                    {
                        if (childRunProperty is XElement childNodePropertyElement)
                        {
                            XElement copyChildElement = new XElement(childNodePropertyElement.Name, childNodePropertyElement.Attributes());
                            var nodeStatus = (string)childNodePropertyElement.Attribute(PtOpenXml.Status);

                            if (rPrChangeRunPropertyElement == null && nodeStatus != null)
                            {
                                rPrChangeRunPropertyElement = new XElement(W.rPr);
                                XElement rPrChange = new XElement(W.rPrChange,
                                    new XAttribute(W.author, settings.AuthorForRevisions),
                                    new XAttribute(W.id, _maxId++),
                                    new XAttribute(W.date, settings.DateTimeForRevisions),
                                    rPrChangeRunPropertyElement);
                                rPrElement.Add(rPrChange);
                            }

                            if (nodeStatus == "Deleted")
                            {
                                rPrChangeRunPropertyElement.Add(copyChildElement);
                            }
                            else
                            {
                                rPrElement.Add(copyChildElement);
                            }
                        }
                        else
                        {
                            rPrElement.Add(childRunProperty);
                        }
                    }
                    return rPrElement;
                }
            ...
            List<object> elementList = grouped
              ...
              List<IGrouping<string, ComparisonUnitAtom>> groupedChildren = g
                  ...
                  .ToList();
                    ...
                    if (ancestorBeingConstructed.Name == W.rPr)
                    {
                        List<object> newChildElements = groupedChildren
                                  .Select(gc =>
                                  {
                        ...
                        var newRunProperties = new XElement(W.rPr,
                              ancestorBeingConstructed.Attributes().Where(a => a.Name.Namespace != PtOpenXml.pt),
                              newChildElements);
                    ...
                    if (ancestorBeingConstructed.Name == W.r)
                    {
                        ...
                        var newRun = new XElement(W.r,
                            ...
                            rPr,
                            newChildElements);
                  /* TODO: var newRun = new XElement(W.r,
                  ancestorBeingConstructed.Attributes().Where(a => a.Name.Namespace != PtOpenXml.pt),
                            newChildElements);*/
                        return newRun;
                    }
